...
// ---------- Xử lý file lớn: đọc theo luồng + chỉ giữ số liệu tổng hợp ----------
// Cột cần dùng (18/56 cột) — bỏ qua phần còn lại để tiết kiệm bộ nhớ
const NEEDED_COLS = [
  "Ngày ct", "Tên khách", "Tên Kênh", "Model", "Tên mặt hàng", "Tên nhóm",
  "Sl xuất bán", "Giá bán tb", "Doanh thu xuất bán", "Thực bán",
  "Tiền vốn thực tế", "Lãi", "Mã CTKM", "Tên ct km", "Tháng",
];
const MAX_PROGRAMS_KEPT = 1500;
const MAX_DIST_PER_PROGRAM = 60;

function normKey(s) { return String(s || "").trim().toLowerCase().replace(/\s+/g, " "); }

// Chấp nhận "1.717.592,59" (VN), "1717592.59" (Anh/Mỹ), "(3,214,565)" (số âm kiểu kế toán), "1.234-" (dấu trừ cuối)
function parseNum(v) {
  if (typeof v === "number") return isFinite(v) ? v : 0;
  let s = String(v == null ? "" : v).trim();
  if (!s) return 0;
  // Số âm dạng ngoặc đơn của Excel: (1.234) nghĩa là -1234
  let negative = false;
...
function emptyBucket() { return { qty: 0, revenue: 0, cost: 0, profit: 0 }; }
function addToBucket(b, r) { b.qty += r.qty; b.revenue += r.revenue; b.cost += r.cost; b.profit += r.profit; }

function createEmptyAgg() {
  return { rowCount: 0, months: [], byMonth: {}, byDist: {}, byModel: {}, byProgram: {} };
}

function aggregateRow(agg, r) {
  agg.rowCount++;
  const month = r.month || "Không rõ tháng";
  if (!agg.byMonth[month]) { agg.byMonth[month] = emptyBucket(); agg.months.push(month); }
  addToBucket(agg.byMonth[month], r);

  const dKey = `${month}|${r.distName}`;
  if (!agg.byDist[dKey]) agg.byDist[dKey] = { month, name: r.distName, ...emptyBucket() };
  addToBucket(agg.byDist[dKey], r);

  const mName = r.modelCode || r.itemName || "(không rõ)";
  const mKey = `${month}|${mName}`;
  if (!agg.byModel[mKey]) agg.byModel[mKey] = { month, name: mName, ...emptyBucket() };
  addToBucket(agg.byModel[mKey], r);

  const pKey = r.ctkmCode || r.ctkmName || "__none__";
  if (!agg.byProgram[pKey]) {
    agg.byProgram[pKey] = { name: r.ctkmName || "Không có CTKM", modelSet: {}, byDist: {}, ...emptyBucket() };
  }
  const prog = agg.byProgram[pKey];
  addToBucket(prog, r);
  prog.modelSet[mName] = 1;
  if (!prog.byDist[r.distName]) prog.byDist[r.distName] = emptyBucket();
  addToBucket(prog.byDist[r.distName], r);
}

// Gộp 2 bộ tổng hợp (khi nhập thêm file mới, dữ liệu cộng dồn)
function mergeAgg(a, b) {
  const out = createEmptyAgg();
  out.rowCount = a.rowCount + b.rowCount;
  const mergeMap = (ka, kb, target) => {
    Object.entries(ka).forEach(([k, v]) => { target[k] = { ...v }; });
    Object.entries(kb).forEach(([k, v]) => {
      if (!target[k]) target[k] = { ...v };
      else addToBucket(target[k], v);
    });
  };
  mergeMap(a.byMonth, b.byMonth, out.byMonth);
  mergeMap(a.byDist, b.byDist, out.byDist);
  mergeMap(a.byModel, b.byModel, out.byModel);
  out.months = [...new Set([...(a.months || []), ...(b.months || [])])];
  Object.entries(a.byProgram).forEach(([k, v]) => { out.byProgram[k] = JSON.parse(JSON.stringify(v)); });
  Object.entries(b.byProgram).forEach(([k, v]) => {
    if (!out.byProgram[k]) { out.byProgram[k] = JSON.parse(JSON.stringify(v)); return; }
    const t = out.byProgram[k];
    addToBucket(t, v);
    Object.assign(t.modelSet, v.modelSet);
    Object.entries(v.byDist).forEach(([d, dv]) => {
      if (!t.byDist[d]) t.byDist[d] = { ...dv };
      else addToBucket(t.byDist[d], dv);
    });
  });
  return out;
}

// Cắt bớt để chắc chắn lưu vừa bộ nhớ trình duyệt
function trimAgg(agg) {
  const progs = Object.entries(agg.byProgram).sort((x, y) => y[1].revenue - x[1].revenue);
  const kept = {};
  progs.slice(0, MAX_PROGRAMS_KEPT).forEach(([k, v]) => {
    const dists = Object.entries(v.byDist).sort((x, y) => y[1].revenue - x[1].revenue).slice(0, MAX_DIST_PER_PROGRAM);
    const models = Object.keys(v.modelSet || {});
    const modelSetCapped = {};
    models.slice(0, 300).forEach((mn) => { modelSetCapped[mn] = 1; });
    kept[k] = { name: v.name, qty: v.qty, revenue: v.revenue, cost: v.cost, profit: v.profit, modelCount: models.length, modelSet: modelSetCapped, byDist: Object.fromEntries(dists) };
  });
  return { ...agg, byProgram: kept, months: [...new Set(agg.months)].sort((a, b) => a.localeCompare(b, "vi", { numeric: true })) };
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ brands, activeBrandId, dataByBrand }));
        setSaveState("saved");
      } catch (e) {
  ...
  const { models, gtCostCatalog, mtCostCatalog, customFields, programs, priceHistory } = bd;
  const actualAgg = bd.actualAgg || createEmptyAgg();
  ...
  function addBrand() {
    ...
    if (!name) return;
    const id = newId();
    const fresh = makeEmptyBrandData(false);
    setBrands((prev) => [...prev, { id, name }]);
    setDataByBrand((prev) => ({ ...prev, [id]: fresh }));
    setNewBrandName("");
    setActiveBrandId(id);
    setActiveProgramId(fresh.programs[0].id);
  }
  ...
  function rowToRecord(get) {
    const dist = normalizeDistName(get("Tên khách"));
    return {
      month: String(get("Tháng") || "").trim() || "Không rõ tháng",
      distName: dist.display,
      modelCode: String(get("Model") || "").trim(),
      itemName: String(get("Tên mặt hàng") || "").trim(),
      qty: parseNum(get("Sl xuất bán")),
      revenue: parseNum(get("Doanh thu xuất bán")),
      cost: parseNum(get("Tiền vốn thực tế")),
      profit: parseNum(get("Lãi")),
      ctkmCode: String(get("Mã CTKM") || "").trim(),
      ctkmName: String(get("Tên ct km") || "").trim(),
    };
  }

  // Đọc CSV theo từng khối nhỏ — không bao giờ nạp cả file vào bộ nhớ
  async function importCsvStreaming(file) {
    const CHUNK = 2 * 1024 * 1024;
    const MAX_CARRY = 20 * 1024 * 1024; // chặn trường hợp file sai định dạng làm phình bộ nhớ

    // Nhận diện bảng mã qua BOM — file lưu sai kiểu (UTF-16/Unicode Text) sẽ báo lỗi rõ ràng
    const bomBuf = await file.slice(0, 4).arrayBuffer();
    const bom = new Uint8Array(bomBuf);
    let encoding = "utf-8";
    let startOffset = 0;
    if (bom[0] === 0xff && bom[1] === 0xfe) { encoding = "utf-16le"; startOffset = 2; }
    else if (bom[0] === 0xfe && bom[1] === 0xff) { encoding = "utf-16be"; startOffset = 2; }
    else if (bom[0] === 0xef && bom[1] === 0xbb && bom[2] === 0xbf) { startOffset = 3; }
    ...
    let offset = startOffset, carry = "", delim = ",", header = null, colIdx = null, skipped = 0;

    const handleLine = (line) => {
      if (!line || !line.trim()) return;
      const cells = splitCsvLine(line, delim);
      if (!header) {
        header = cells.map((c) => normKey(c.replace(/^\ufeff/, "")));
        colIdx = {};
        NEEDED_COLS.forEach((name) => {
          const i = header.indexOf(normKey(name));
          if (i >= 0) colIdx[name] = i;
        });
        if (colIdx["Model"] === undefined && colIdx["Tên mặt hàng"] === undefined) {
          throw new Error(`Không tìm thấy cột "Model" hoặc "Tên mặt hàng" trong file. Các cột đọc được: ${header.slice(0, 60).join(", ")}`);
        }
        return;
      }
      const get = (name) => {
      ...
      const rec = rowToRecord(get);
      if (!rec.modelCode && !rec.itemName) { skipped++; return; }
      aggregateRow(agg, rec);
    };
    ...
    while (offset < file.size) {
      ...
      if (carry.length > MAX_CARRY) {
        throw new Error("File không đúng định dạng CSV (không tìm thấy dấu xuống dòng). Hãy mở lại trong Excel và lưu bằng: Save As → CSV UTF-8 (*.csv).");
      }
      if (header === null && lines.length) delim = detectDelimiter(lines[0]);
      for (const ln of lines) handleLine(ln);
      offset += CHUNK;
      setActualsImportMsg(`Đang đọc... ${Math.min(100, Math.round(((offset - startOffset) / file.size) * 100))}% — đã xử lý ${agg.rowCount.toLocaleString("vi-VN")} dòng`);
      await new Promise((r) => setTimeout(r, 0));
    }
    if (carry) handleLine(carry);
    return { agg, skipped, header };
  }

  async function importXlsxWhole(file) {
    const buf = await file.arrayBuffer();
    ...
    const keyMap = {};
    headerKeys.forEach((k) => { keyMap[normKey(k)] = k; });
    const agg = createEmptyAgg();
    let skipped = 0;
    rows.forEach((row) => {
      const get = (name) => {
        const k = keyMap[normKey(name)];
        return k !== undefined && row[k] !== undefined ? row[k] : "";
      };
      const rec = rowToRecord(get);
      if (!rec.modelCode && !rec.itemName) { skipped++; return; }
      aggregateRow(agg, rec);
    });
    return { agg, skipped, header: headerKeys };
  }
  ...
  async function handleImportActuals(e) {
    ...
    if (!isCsv && sizeMB > 12) {
      setActualsImportMsg(
        `File Excel nặng ${sizeMB.toFixed(0)}MB — trình duyệt sẽ hết bộ nhớ nếu cố mở (file .xlsx khi giải nén ra để đọc thường phình lên gấp 8–10 lần). ` +
        `Cách xử lý: mở file trong Excel → File → Save As → chọn định dạng "CSV UTF-8 (*.csv)" → tải file .csv lên đây. ` +
        `App đọc CSV theo từng khối nhỏ nên xử lý được file vài trăm MB mà không bị treo.`
      );
      e.target.value = "";
      return;
    }
    ...
    try {
      const { agg, skipped, header } = isCsv ? await importCsvStreaming(file) : await importXlsxWhole(file);

      if (agg.rowCount === 0) {
        setActualsImportMsg(`Không đọc được dòng nào khớp cột "Model"/"Tên mặt hàng". Các cột phát hiện được: ${(header || []).slice(0, 60).join(", ")}`);
        return;
      }

      const merged = trimAgg(mergeAgg(actualAgg, agg));
      try {
        updateBrand({ actualAgg: merged });
        setActualsImportMsg(`Nhập xong ${agg.rowCount.toLocaleString("vi-VN")} dòng (${merged.months.join(", ")}). Bỏ qua ${skipped.toLocaleString("vi-VN")} dòng không có Model.`);
      } catch (storageErr) {
        console.error(storageErr);
        setActualsImportMsg("Đọc xong nhưng không lưu được do vượt dung lượng trình duyệt. Thử nhập từng tháng một.");
      }
    } catch (err) {
      console.error(err);
      setActualsImportMsg(`Không đọc được file: ${err && err.message ? err.message : "lỗi không xác định"}. Nếu file rất lớn, hãy lưu sang định dạng CSV UTF-8 rồi thử lại.`);
    } finally { e.target.value = ""; }
  }
...
Nếu không chắc model/chi phí nào người dùng nhắc tới, chọn model/chi phí có tên gần đúng nhất trong danh mục. Không tự bịa id không có trong danh mục.`;
  ...
  // ---------- Actuals aggregation ----------
  const availableMonths = useMemo(() => (actualAgg.months || []), [actualAgg]);
  const totalRows = actualAgg.rowCount || 0;

  const aggByDist = useMemo(() => {
    const g = {};
    Object.values(actualAgg.byDist || {}).forEach((v) => {
      if (monthFilter !== "all" && v.month !== monthFilter) return;
      if (!g[v.name]) g[v.name] = { qty: 0, revenue: 0, cost: 0, profit: 0 };
      addToBucket(g[v.name], v);
    });
    return g;
  }, [actualAgg, monthFilter]);

  const aggByModel = useMemo(() => {
    const g = {};
    Object.values(actualAgg.byModel || {}).forEach((v) => {
      if (monthFilter !== "all" && v.month !== monthFilter) return;
      if (!g[v.name]) g[v.name] = { qty: 0, revenue: 0, cost: 0, profit: 0 };
      addToBucket(g[v.name], v);
    });
  ...
  const aggByProgram = useMemo(() => actualAgg.byProgram || {}, [actualAgg]);

  const monthlyTrend = useMemo(
    () => Object.entries(actualAgg.byMonth || {}).sort((a, b) => a[0].localeCompare(b[0], "vi", { numeric: true })),
    [actualAgg]
  );
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", color: BLACK, maxWidth: 1360, margin: "0 auto", padding: 20 }}>
      ...
      <nav style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <TabButton active={tab === "actuals"} onClick={() => setTab("actuals")} icon={<BarChart3 size={16} />} label={`Dữ liệu thực tế (${totalRows.toLocaleString("vi-VN")})`} />
        ...
        <section>
          <div style={{ border: "1px solid #eee", borderRadius: 10, padding: 14, marginBottom: 18, background: "#fafafa" }}>
            ...
            <div style={{ fontSize: 12, color: "#666", marginBottom: 8 }}>
              Đọc đúng cột file gốc. Có thể nhập nhiều file/nhiều tháng, dữ liệu cộng dồn.
              <br />
              <strong>File lớn (trên ~50.000 dòng):</strong> mở trong Excel → <em>Save As</em> → chọn <strong>CSV UTF-8 (*.csv)</strong> rồi tải file .csv lên. App đọc CSV theo từng khối nhỏ nên xử lý được file hàng trăm nghìn dòng mà không bị treo.
            </div>
                  ...
                  return (
                    <div key={code} style={{ border: "1px solid #eee", borderRadius: 8, marginBottom: 8 }}>
                      <button onClick={() => setExpandedProgram(isOpen ? null : code)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "10px 12px", background: "#fafafa", border: "none", cursor: "pointer", textAlign: "left" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5 }}>{isOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<strong>{v.name}</strong><span style={{ color: "#999", fontSize: 11 }}>({distEntries.length} NPP · {v.modelCount || 0} model)</span></span>
          ...
          {totalRows === 0 && <div style={{ fontSize: 13, color: "#888" }}>Chưa có dữ liệu thực tế. Upload file báo cáo tháng ở trên để bắt đầu.</div>}
        </section>
      )}
