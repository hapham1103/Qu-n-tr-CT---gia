...
export default function App() {
  ...
  async function handleImportActuals(e) {
    ...
    try {
      ...
      // Khớp tên cột không phân biệt hoa/thường, khoảng trắng thừa — tránh lỗi 0 dòng do sai khác nhỏ trong tên cột
      const normalizeKey = (s) => String(s || "").trim().toLowerCase().replace(/\s+/g, " ");
      const headerKeys = rows.length ? Object.keys(rows[0]) : [];
      const keyMap = {};
      headerKeys.forEach((k) => { keyMap[normalizeKey(k)] = k; });
      const G = (row, name) => {
        const actualKey = keyMap[normalizeKey(name)];
        return actualKey !== undefined && row[actualKey] !== undefined ? row[actualKey] : "";
      };
      const asDate = (v) => {
      ...
      if (parsed.length === 0 && rows.length > 0) {
        setActualsImportMsg(`Đọc được ${rows.length} dòng nhưng không dòng nào khớp cột "Model"/"Tên mặt hàng". Các cột phát hiện được trong file: ${headerKeys.join(", ")}`);
      } else {
